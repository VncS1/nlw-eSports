...
export function CreateAdModal() {
    ...
    async function handleCreateAd(event: FormEvent) {
        ...
        if(!data.name){ //Validação simples pra n enviar o form vazio
            return
        }
        ...
        try {
            await axios.post(`http://localhost:3333/games/${data.game}/ads`, {
                name: data.name,
                yearsPlaying: Number(data.yearsPlaying), //Convertendo para number
                discord: data.discord,
                weekDays: weekDays.map(Number), //Convertendo em numero desse jeito por ser um array
    ...
    return (
        <Dialog.Portal>
            ...
            <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
                <Dialog.Title className="text-3xl tex-white font-black">Publique um anúncio</Dialog.Title>
